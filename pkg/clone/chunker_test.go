...
func TestChunker(t *testing.T) {
	...
	assert.NoError(t, err)
	tables, err := LoadTables(ctx, source.Type, conns[0], "customer", true, nil, 1*time.Second)
	assert.NoError(t, err)
...
func TestChunkerSingleRow(t *testing.T) {
	...
	conns, err := OpenConnections(ctx, db, 1)
	assert.NoError(t, err)
	tables, err := LoadTables(ctx, source.Type, conns[0], "customer", true, []string{"customers"}, 1*time.Second)
	assert.NoError(t, err)
	err = GenerateTableChunks(ctx, conns[0], tables[0], 10, 1*time.Second, chunks)
	assert.NoError(t, err)
	close(chunks)
