...
func writeBatch(ctx context.Context, config WriterConfig, batch Batch, db *sql.DB, retry RetryOptions) (err error) {
	writesRequested.WithLabelValues(batch.Table.Name, string(batch.Type)).Add(float64(len(batch.Rows)))

	err = doWriteBatch(ctx, config, batch, db, retry)
	...
	if err != nil {
		...
		if len(batch.Rows) > 1 {
			batch1, batch2 := splitBatch(batch)
			err = writeBatch(ctx, config, batch1, db, retry)
			if err != nil {
				return errors.WithStack(err)
			}
			err = writeBatch(ctx, config, batch2, db, retry)
			if err != nil {
				return errors.WithStack(err)
			}
			return nil
...
func doWriteBatch(ctx context.Context, config WriterConfig, batch Batch, db *sql.DB, retry RetryOptions) (err error) {
	logger := log.WithField("task", "writer").WithField("table", batch.Table.Name)
