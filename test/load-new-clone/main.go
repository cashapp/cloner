...
func main() {
	...
	wg.Add(1)
	replicate.Stderr = OnFirstOutput(os.Stderr, "starting new replication", func() {
		go func() {
			wg.Done()
		}()
	})
	NoErr(replicate.Start())
	wg.Wait()

	// Do a snapshot with multiple repair attempts
	snapshot := exec.Command(cloner,
		"new-clone",
		"--metrics-port", "9103",
		"--source-host", "127.0.0.1",
		"--source-database", "tpcc",
		...
		"--ignore-tables", "history",
		"--repair-attempts", "10")
	snapshot.Stdout = os.Stdout
	cloneStart := OnFirstOutput(os.Stderr, "starting clone", func() {
		go func() {
			wg.Add(1)
			defer wg.Done()
	...
	cloneDone := OnFirstOutput(cloneStart, "clone done", func() {
		go func() {
			wg.Add(1)
			defer wg.Done()
